document.addEventListener("DOMContentLoaded", function () {
    ...
    // ---- MARCAR ELEMENTO ACTIVO (SUBRAYADO) EN EL MENÚ ----
    ...
    const pagoTarjeta = document.getElementById('pago-tarjeta-field');
    
    const inputTarjetaNum = document.getElementById('chk-tarjeta-num');
    const inputTarjetaCad = document.getElementById('chk-tarjeta-cad');
    const inputTarjetaCvv = document.getElementById('chk-tarjeta-cvv');
    const inputBancoNombre = document.getElementById('chk-banco-nombre');
    const inputBancoApellidos = document.getElementById('chk-banco-apellidos');
    ...
    pagoRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            ...
            if (pagoTarjeta) pagoTarjeta.style.display = 'none';
            ...
            if (inputTarjetaNum) inputTarjetaNum.removeAttribute('required');
            if (inputTarjetaCad) inputTarjetaCad.removeAttribute('required');
            if (inputTarjetaCvv) inputTarjetaCvv.removeAttribute('required');
            if (inputBancoNombre) inputBancoNombre.removeAttribute('required');
            if (inputBancoApellidos) inputBancoApellidos.removeAttribute('required');

            if (e.target.value === 'tarjeta') {
                if (pagoTarjeta) pagoTarjeta.style.display = 'block';
                if (inputTarjetaNum) inputTarjetaNum.setAttribute('required', 'true');
                if (inputTarjetaCad) inputTarjetaCad.setAttribute('required', 'true');
                if (inputTarjetaCvv) inputTarjetaCvv.setAttribute('required', 'true');
            } else if (e.target.value === 'bizum') {
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener("submit", async function (e) {
            ...
            if (!confirm('¿Estás seguro/a de la compra?')) return;
            ...
            let datos_pago = '';
            let titular_banco_nombre = null;
            let titular_banco_apellidos = null;
            if (metodo_pago === 'bizum') datos_pago = inputBizum ? inputBizum.value : '';
            else if (metodo_pago === 'banco') {
                datos_pago = inputBanco ? inputBanco.value : '';
                titular_banco_nombre = inputBancoNombre ? inputBancoNombre.value : null;
                titular_banco_apellidos = inputBancoApellidos ? inputBancoApellidos.value : null;
            }
            else if (metodo_pago === 'paypal') datos_pago = inputPaypal ? inputPaypal.value : '';
            else if (metodo_pago === 'tarjeta') {
                const tNum = inputTarjetaNum ? inputTarjetaNum.value : '';
                const tCad = inputTarjetaCad ? inputTarjetaCad.value : '';
                datos_pago = `Tarjeta: **** **** **** ${tNum.slice(-4)} (Cad: ${tCad})`;
            }
            ...
            const data = {
                ...
                titular_banco_nombre: titular_banco_nombre,
                titular_banco_apellidos: titular_banco_apellidos,
                total: total,
                items: cart
            };
    ...
    if (contactForm) {
        contactForm.addEventListener("submit", async function (e) {
            ...
            try {
                ...
                if (response.ok) {
                    showToast("Gracias por tu mensaje, " + data.nombre + " " + data.apellidos + "! Pronto nos pondremos en contacto.");
                    contactForm.reset();
                } else {
                    const err = await response.json();
